...
bool OpenCLDeviceManager::transferToDevice(Tensor &tensor, void *context) {
  runtime::OpenCLDeviceTransferContext *ctx = nullptr;
  if (context == nullptr) {
    cl_mem buffer;
    if (auto autoDeviceBufferOrErr =
            allocDeviceBuffer(tensor.getSizeInBytes())) {
      buffer = *autoDeviceBufferOrErr;
    } else {
      return false;
    }
    ctx = new OpenCLDeviceTransferContext(buffer, 0);
  } else {
    ctx = static_cast<runtime::OpenCLDeviceTransferContext *>(context);
  }
  ...
  tensor.moveToDevice((DeviceManager *)this, ctx);
  return true;
}
