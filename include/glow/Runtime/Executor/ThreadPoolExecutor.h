...
public:
  /// Constructor.
  explicit ExecutionState(RunIdentifierTy id, const Schedule &schedule,
  ...
  getUniqueNodeContextPtr(const Schedule::Task *node);
  ...
  /// otherwise.
  bool incrementNodeParentsDone(const Schedule::Task *node,
...
private:
  ...
  std::unordered_map<const Schedule::Task *, std::atomic<unsigned>>
      nodeParentsDone_;
  /// Input contexts for all of the nodes. These are gradually
  /// populated as a node's parents finish.
  std::unordered_map<const Schedule::Task *, std::unique_ptr<ExecutionContext>>
      inputCtxs_;
...
public:
  ...
  void run(const Schedule &schedule, std::unique_ptr<ExecutionContext> context,
